...
console.log("=== v3.7: 画像変換の縮小率プリセット(90/80/70/60/50%) ===");
{
  S.widthLock=0; // 幅ロックの影響を受けない状態で検証
  const conv=w.eval("conv");
  const gw=20, gh=10;
  conv.gray=new Float32Array(gw*gh).fill(50); // 全ピクセルがしきい値未満(=被写体全面)
  conv.gw=gw; conv.gh=gh;
  w.document.getElementById("convThr").value="160";
  w.document.getElementById("convBudget").value="200";
  w.document.getElementById("convScale").value="100";
  w.eval("convAutofit")();
  const base=conv.baseCols;
  eq("予算内に収まる基準サイズが求まる", base>=4 && base<=44, true);
  eq("100%は基準サイズと一致", +w.document.getElementById("convCols").value, base);

  w.document.getElementById("convScale").value="50";
  w.document.getElementById("convScale").dispatchEvent(new w.Event("change"));
  eq("50%は基準の半分(四捨五入・下限4)", +w.document.getElementById("convCols").value, Math.max(4,Math.round(base*0.5)));

  w.document.getElementById("convScale").value="70";
  w.document.getElementById("convScale").dispatchEvent(new w.Event("change"));
  eq("70%は基準の7割", +w.document.getElementById("convCols").value, Math.max(4,Math.round(base*0.7)));

  w.document.getElementById("convScale").value="90";
  w.document.getElementById("convScale").dispatchEvent(new w.Event("change"));
  eq("90%は基準の9割", +w.document.getElementById("convCols").value, Math.max(4,Math.round(base*0.9)));

  // 道具箱を開き直すと縮小率は毎回100%から選び直せる(前回の縮小率を引きずらない)
  w.document.getElementById("btnConv").click();
  eq("道具箱を開くと縮小率は100%に戻る", w.document.getElementById("convScale").value, "100");
  w.document.getElementById("convClose").click();
}
